                                ...
                                <table class="table table-bordered table-striped dataTable table-hover" id="tablaUsuarios" class="display">
                                    ...
                                    <c:forEach var="user" items="${usuarios}" varStatus="iteracion">                                                    
                                        <tr>
                                            ...
                                            <td>${user.getEmail()}</td>
                                            <td>${user.getClave_usuario()}</td>
                                            <td>${user.getEstado_usuario()}</td>
                                            <c:if test="${user.getId_rol_fk() eq '1'}" >
                                                <td class="text-success ">Gerente</td>
                                            </c:if>
                                            <c:if test="${user.getId_rol_fk() eq '2'}" >
                                                <td class="text-success ">Cliente</td>
                                            </c:if>
                                            <c:if test="${user.getId_rol_fk() eq '3'}" >
                                                <td class="text-success ">Almacenista</td>
                                            </c:if>
                                            <c:if test="${user.getId_rol_fk() eq '4'}" >
                                                <td class="text-success ">Repartidor</td>
                                            </c:if>
                                            <c:if test="${user.getId_rol_fk() eq '5'}" >
                                                <td class="text-success ">Atencion al cliente</td>
                                            </c:if>

                                            <td><a class="btn btn-warning" href="#">Editar</a>

                                                <c:if test="${user.getEstado_usuario() eq '0'}" >

                                                    <input type="hidden" name="id" value="${user.getId_usuario()}">
                                                    <button class="btn-transparetn border-0 bg-transparent py-2"
                                                            onclick="alertaInactivarU(${user.getId_usuario()}, '0')">
                                                        <i class="fas fa-truck-loading" style="font-size:25px;"></i></button>

                                                </c:if>
                                                <c:if test="${user.getEstado_usuario() eq '1'}" >

                                                    <input type="hidden" name="id" value="${user.getId_usuario()}">
                                                    <button class="btn-transparetn border-0 bg-transparent py-2"
                                                            onclick="alertaActivarU(${user.getId_usuario()}, '1')">
